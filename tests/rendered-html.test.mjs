...
test("首頁對照表以四個分類呈現完整29種樂器，中英文同列並保留縮寫", async () => {
  ...
  assert.match(html, /<caption>鼓類/);
  assert.match(html, /<caption>鈸與鑼類/);
  assert.match(html, /<caption>琴類/);
  assert.match(html, /<caption>小樂器/);
...
test("管樂團知識頁提供三個可點選配置區域", async () => {
  ...
  assert.match(html, /images\/band\/cutouts\/electric-bass.webp/);
...
test("演奏基本功顯示三種對稱式握法與簡易示意圖", async () => {
  ...
  for (const grip of ["german", "french", "american"]) assert.match(html, new RegExp(`/images/grips/${grip}.webp`));
